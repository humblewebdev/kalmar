...
class ValidFluent {
	...
	private static $error_duplicateUsername = 'Please enter a different username';
	private static $error_duplicateSlug = 'Please no duplicate slugs.';
	private static $error_duplicateEmail = 'That email has already been used to register';
	private static $error_recaptcha = 'The reCAPTCHA wasn\'t entered correctly.';
	// some regEx's
	 ...
	 * @param string $params
	 *        	extra parameter to default error message
	 */
	...
	function oneOf($items, $errorMsg = NULL) {
		if ($this->isValid && (! empty ( $this->currentObj->value ))) {
			...
			$result = array_intersect ( $item, array (
					strtolower ( $this->currentObj->value ) 
			) );
	...
	/**
	 * Only allows A-Z a-z 0-9 space and ( - .
	 * _ )
	 * 
	 * @param string $errorMsg        	
	 * @return ValidFluent
	...
	function alfaNum($errorMsg = NULL) {
		if ($this->isValid && (! empty ( $this->currentObj->value ))) {
			$this->isValid = (preg_match('#[0-9]|[a-z]#i', $this->currentObj->value )) ? TRUE : FALSE;
			if (! $this->isValid)
				$this->setErrorMsg ( $errorMsg, self::$error_alfa );
		}
		return $this;
	...
	function newSlug_duplicate($errorMsg = Null) {
		if ($this->isValid && (! empty ( $this->currentObj->value ))) {
			//require_once "./db/db.class.php";
			$slug = mysql_real_escape_string ($this->currentObj->value);
			$slug = str_replace(' ', '-', $slug);
			$slug = strtolower($slug);
			$pdo = PDOConnection::instance ();
			$db = $pdo->getConnection ( 'mysql:host=localhost;dbname=cmk_database', 'root', 'root' );
			$sql = 'SELECT slug FROM product_attributes WHERE slug = ?';
			$stmt = $db->prepare ( $sql );
			
			$stmt->bindParam( 1, $slug, PDO::PARAM_STR, 255 );
			$stmt->execute();
			$result = $stmt->fetchAll();
			$this->isValid = (count($result)>=1) ? FALSE : TRUE;
		}
		if (! $this->isValid ()) {
			$this->setErrorMsg ( $errorMsg, self::$error_duplicateSlug );
		}
	}
	
	function updateSlug_duplicate($id=null, $errorMsg = Null) {
		if ($this->isValid && (! empty ( $this->currentObj->value ))) {
			//require_once "./db/db.class.php";
			$slug = mysql_real_escape_string ($this->currentObj->value);
			$slug = str_replace(' ', '-', $slug);
			...
			$pdo = PDOConnection::instance ();
			$db = $pdo->getConnection ( 'mysql:host=localhost;dbname=cmk_database', 'root', 'root' );
			$sql = 'SELECT slug FROM product_attributes WHERE (slug = ? AND id <> ?)';
			$stmt = $db->prepare ( $sql );
				
			$stmt->bindParam( 1, $slug, PDO::PARAM_STR, 255 );
			$stmt->bindParam(2, $id, PDO::PARAM_STR, 255);
			$stmt->execute();
			$result = $stmt->fetchAll();
			$this->isValid = (count($result)>=1) ? FALSE : TRUE;
		}
	...
	function email_duplicate($errorMsg = null) {
		if ($this->isValid && (! empty ( $this->currentObj->value ))) {
			require_once "./db/db.class.php";
			$email = mysql_real_escape_string ( $this->currentObj->value );
			$pdo = PDOConnection::instance ();
			$db = $pdo->getConnection ( 'mysql:host=localhost;dbname=America', 'root', 'root' );
			$sql = "SELECT login_email FROM users WHERE login_email = ?";
			$stmt = $db->prepare ( $sql );
			
			$stmt->bindParam ( 1, $email, PDO::PARAM_STR, 255 );
			$stmt->execute ();
			$this->isValid = ($result = $stmt->fetchAll ( PDO::FETCH_ASSOC )) ? FALSE : TRUE;
		}
		if (! $this->isValid ()) {
			$this->setErrorMsg ( $errorMsg, self::$error_duplicateEmail );
		}
	}
	
	function recaptcha_valid($errorMsg = null){
		if($this->isValid()){
			require_once('./classes/lib/recaptchalib.php');
			$privatekey = "6LcVk-USAAAAABhd-CsEWCPw5NqjscMbz2CQqrN8";
			...
			if (!$resp->is_valid) {
				...
				$this->setErrorMsg ( $errorMsg, self::$error_recaptcha );
